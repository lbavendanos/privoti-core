...
it('returns paginated product categories', function () {
    ProductCategory::factory()->count(30)->create();

    /** @var LengthAwarePaginator<int, ProductCategory> $result */
    $result = app(GetProductCategoriesAction::class)->handle();
...
it('returns product categories filtered by name', function () {
    /** @var Collection<int, ProductCategory> $categories */
    $categories = ProductCategory::factory()->count(5)->create();
    /** @var ProductCategory $firstProductCategory */
    $firstProductCategory = $categories->first();
    ...
    /** @var ProductCategory $categoryFound */
    $categoryFound = $result->first();

    expect($categoryFound->id)->toBe($firstProductCategory->id)
        ->and($categoryFound->name)->toBe($firstProductCategory->name);
});

it('returns product categories filtered by a single :dataset date', function (string $field) {
...
it('returns product categories filtered by :dataset date range', function (string $field) {
    /** @var List<CarbonImmutable> $mockDates */
    $mockDates = [
        now()->subDays(5),
        now()->subDays(5),
...
it('returns product categories ordered by :dataset ascending', function (string $field) {
...
it('returns product categories ordered by :dataset descending', function (string $field) {
    ProductCategory::factory()->count(10)->create();
